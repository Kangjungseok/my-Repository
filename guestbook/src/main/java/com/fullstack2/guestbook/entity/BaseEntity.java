...
/*
 * BaseEntity : 얘 자체가 Entity가 되는 목적이 아니라 특정 속성을 상속해주는 목적으로
 * 선언되는 엔티티입니다.
 * 추상으로 선언만 한다고 entity가 안되는게 아니고, 반드시 @MappedSuperClass 라고
 * 선언을 해야만 베인티티화 되어집니다.
 * 얘가 하는 일은 상속도 있지만, 메모리에 빈즈로 머물다가 엔티티에 변화가 생기면 즉각
 * 이를 하위 엔티티가 영속계층(DB 쪽) 반영하도록 특정 컬럼이나, 메서드를 보유합니다.
 * 지금은 날짜값을 자동으로 insert, update 만 하도록 컬럼 설정을 하고, 어노테이션 선언합니다.
 * 그럼 하위 엔티티는 날짜와 관련된 부분은 DB에 따로 작업을 하지 않아도 됩니다.
 * 단 updatable, insertable 이라는 속성을 false로 줘야만 합니다. 
 * 이건 어노테이션이 따로 존재하니 거기서 설명할게요.
 */
@MappedSuperclass //이렇게 선언하면 이 엔티티는 테이블화 되지 않습니다.
//엔티티를 관리하는 컨텍스트에 이 엔티티의 컬럼값이 변화가 생겼는지를 관리하는 리스너 등록
//이렇게 하면 변경 시 즉각 엔티티에 통보를 하게되고, 이 값을 하위 엔티티를 통해 DB에 전달하도록 하는 역할
//주의!!! 이 어노테이션이 활성화되려면, 프로젝트의 메인클래스에 이 리스너를 등록하는 
//어노테이션(EnableJpaAuditing)을 반드시 선언해야 합니다.
